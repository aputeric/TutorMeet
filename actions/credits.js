...
export async function checkAndAllocateCredits(user) {
  try {
    ...
    // Only allocate credits for students
 ...
 */
export async function deductCreditsForAppointment(userId, tutorId) {
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      // Create transaction record for tutor (addition)
      await tx.creditTransaction.create({
        data: {
          userId: tutor.id,
          amount: APPOINTMENT_CREDIT_COST,
      ...
      // Update tutor's credit balance (increment)
