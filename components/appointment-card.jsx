...
export function AppointmentCard({
  ...
  const canMarkCompleted = () => {
    if (userRole !== "TUTOR" || appointment.status !== "SCHEDULED") {
      return false;
    }
    const now = new Date();
  ...
  // Handle save notes (tutor only)
  const handleSaveNotes = async () => {
    if (notesLoading || userRole !== "TUTOR") return;
  ...
  const otherParty =
    userRole === "TUTOR" ? appointment.student : appointment.tutor;

  const otherPartyLabel = userRole === "TUTOR" ? "Student" : "tutor";
  const otherPartyIcon = userRole === "TUTOR" ? <User /> : <GraduationCap />;
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
            {/* LEFT COLUMN: Other party, Scheduled Time, Status */}
            ...
            {/* RIGHT COLUMN: Student Description, Video Consultation, Tutor Notes */}
            <div className="space-y-4">
              {/* Student Description */}
              {appointment.studentDescription && (
                <div className="space-y-2">
                  <h4 className="text-sm font-medium text-muted-foreground">
                    {userRole === "TUTOR" ? "Student Description" : "Your Description"}
                  </h4>
              ...
              {/* Tutor Notes (Tutor can view/edit, Student can only view) */}
              <div className="space-y-2">
          ...
          <DialogFooter className="flex flex-col-reverse sm:flex-row sm:justify-between sm:space-x-2">
            <div className="flex gap-2">
              {/* Mark as Complete Button - Only for TUtors */}
              {canMarkCompleted() && (
                <Button
                  onClick={handleMarkCompleted}
                  disabled={completeLoading}
